...
app = FastAPI(title="archie-brain-api", version="0.1.0")
...
QDRANT_COLLECTION = os.environ.get("QDRANT_COLLECTION", "archie_knowledge_bge_m3_v1")
...
def search(req: SearchRequest, authorization: Optional[str] = Header(default=None)):
    ...
    article_ids = [
        h.get("payload", {}).get("article_id")
        for h in hits
        if h.get("payload", {}).get("article_id")
    ]
